...
class TestPromptCachingIntegration:
    ...
    @pytest.mark.asyncio
    async def test_caching_endpoint_exists(self, test_client: TestClient):
    ...
    async def test_caching_reads_cache_on_second_call(
        ...
        with patch("clients.anthropic_client.AnthropicClient.validate_dte",
            ...
            # First call
            status1, response1 = await asyncio.to_thread(
                self._make_validation_request,
                test_client,
                sample_dte_data
            )
            ...
            status2, response2 = await asyncio.to_thread(
                self._make_validation_request,
                test_client,
                sample_dte_data
            )
    ...
    async def test_cache_control_header_in_system_messages(self):
        ...
        # Mock the client's underlying Anthropic API
    ...
    async def test_caching_error_handling(
        ...
        with patch("clients.anthropic_client.AnthropicClient.validate_dte",
            ...
            assert status in [400, 500]  # Error response
