...
Run standalone with system Python (no Odoo env needed):
    ...
    python addons/localization/l10n_cl_dte/tests/smoke/smoke_xsd_dte33.py
...
def module_root() -> str:
    # This file: addons/localization/l10n_cl_dte/tests/smoke/smoke_xsd_dte33.py
...
def get_paths():
    ...
    fixture_path = os.path.join(root, 'tests', 'fixtures', 'dte33_factura.xml')
...
def main() -> int:
    ...
    print(f"[XSD SMOKE] DTE 33 (Factura Electrónica): {'✅ PASS' if ok else '❌ FAIL'}")
    if not ok:
        print(err)
