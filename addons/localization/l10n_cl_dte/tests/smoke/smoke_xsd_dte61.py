# -*- coding: utf-8 -*-
"""
Smoke XSD validation for DTE 61 (Nota de Crédito)

Run standalone with system Python (no Odoo env needed):

    python addons/localization/l10n_cl_dte/tests/smoke/smoke_xsd_dte61.py
...
def module_root() -> str:
    # This file: addons/localization/l10n_cl_dte/tests/smoke/smoke_xsd_dte61.py
...
def get_paths():
    ...
    fixture_path = os.path.join(root, 'tests', 'fixtures', 'dte61_nota_credito.xml')
...
def main() -> int:
    ...
    print(f"[XSD SMOKE] DTE 61 (Nota de Crédito): {'✅ PASS' if ok else '❌ FAIL'}")
    if not ok:
        print(err)
