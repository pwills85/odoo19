# -*- coding: utf-8 -*-
"""
Smoke XSD validation for DTE 34 (Factura Exenta)

Run standalone with system Python (no Odoo env needed):

    python addons/localization/l10n_cl_dte/tests/smoke/smoke_xsd_dte34.py
...
def module_root() -> str:
    # This file: addons/localization/l10n_cl_dte/tests/smoke/smoke_xsd_dte34.py
...
def get_paths():
    ...
    fixture_path = os.path.join(root, 'tests', 'fixtures', 'dte34_factura_exenta.xml')
...
def main() -> int:
    ...
    print(f"[XSD SMOKE] DTE 34 (Factura Exenta): {'✅ PASS' if ok else '❌ FAIL'}")
    if not ok:
        print(err)
